...
const MySQLDSN = "root:123456@(8.142.129.250:33060)/zhjs?charset=utf8mb4&parseTime=True&loc=Local"
...
func GenGenerate() {
	...
	//Joinnum := g.GenerateModel("avt_joinnum")
	//// 创建全部模型文件, 并覆盖前面创建的同名模型
	allModel := g.GenerateAllTable(fieldOpts...)
	//
	//keytag := make(map[string][]string)
	//keytag["foreignKey"] = []string{"room_id"}
	//// 创建user和unit的关联模型
	//Room := g.GenerateModel("avt_room",
	//	append(
	//		fieldOpts,
	//		// user 一对多 address 关联, 外键`uid`在 address 表中
	//		gen.FieldRelate(field.BelongsTo, "Joinnum", Joinnum, &field.RelateConfig{GORMTag: keytag}),
	//	)...,
	//)
	//// 创建模型的方法,生成文件在 query 目录; 先创建结果不会被后创建的覆盖
	//g.ApplyBasic(Room, Joinnum)
